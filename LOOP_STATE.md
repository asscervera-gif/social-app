...
- **Notificaciones locales del sistema — RESUELTAS por completo en ambas
  ...
  para el detalle). Pendiente real: push verdadero (FCM/APNs o
  alternativa libre tipo UnifiedPush) para cuando el proceso no está
  vivo — sigue siendo el hueco grande de infraestructura, no resuelto por
  esto ni pretendía estarlo.
...
- iOS sigue sin compilar nunca en este entorno — cualquier cambio ahí es
  "razonado pero no verificado", debe llevar el aviso de honestidad
  correspondiente si toca una API externa no confirmada.
