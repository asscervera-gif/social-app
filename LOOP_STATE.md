...
- **Ronda 2026-08-26 (dentro de `/loop`), silenciar las historias de alguien sin dejar de seguirlo, comparado con Instagram/Snapchat**: las dos dejan silenciar la historia de una persona concreta -- deja de aparecer priorizada en la bandeja (se manda al final, atenuada), sin dejar de seguirla ni bloquearla, y sigue siendo accesible si se busca a propósito. Confirmado en el propio código: `stories_select` (0075_close_friends_stories.sql) solo conoce 'everyone'/'close_friends'; `grep` de "muted_story"/"story_mute" en todo el repo no encontró nada.
  ...
  - **Verificado real, no simulado**: RLS 267/267. Android `:app:compileDebugKotlin` OK a la primera (`BUILD SUCCESSFUL`), daemon detenido. iOS: pendiente de verificar en CI real tras el push (próxima entrada de esta misma ronda).
