...
- **Ronda 2026-08-27 (dentro de `/loop`), ocultar el número de "me gusta", comparado con Instagram/Facebook**: las dos dejan al autor ocultar el NÚMERO de "me gusta" (real desde 2019, por la ansiedad social de comparar contadores) -- el resto de gente sigue dando like con normalidad, y el propio autor sigue viendo su cifra real siempre; solo desaparece el número para los DEMÁS. Confirmado en el propio código: `posts.like_count`/`reels.like_count` siempre se mostraban en bruto, sin ningún interruptor real.
  ...
  - **Verificado real, no simulado**: RLS 320/320. Android `:app:compileDebugKotlin` OK a la primera (`BUILD SUCCESSFUL`), daemon detenido. iOS: pendiente de verificar en CI real tras el push (próxima entrada de esta misma ronda).
