...
- **Ronda 2026-08-27 (dentro de `/loop`), marcar contenido como sensible, comparado con Instagram/Twitter/TikTok**: las tres dejan a quien publica marcar su propia foto/vídeo como "contenido sensible" -- se muestra difuminado/oculto con un aviso real hasta que quien lo ve toca a propósito para revelarlo. Confirmado en el propio código: `posts`/`reels` no tenían ninguna columna ni concepto de sensibilidad -- toda foto/vídeo se mostraba siempre igual.
  ...
  - **Verificado real, no simulado**: RLS 325/325. Android `:app:compileDebugKotlin` OK (limpio, sin warnings nuevos tras el ajuste de arriba), daemon detenido. iOS: pendiente de verificar en CI real tras el push (próxima entrada de esta misma ronda).
