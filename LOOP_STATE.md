...
- **Ronda 2026-08-27/28 (dentro de `/loop`), adhesivo de pregunta en una historia ("Pregúntame algo"), comparado con Instagram**: Instagram deja añadir una caja de pregunta a una historia real -- cualquiera que la vea puede responder en privado (solo quien publicó la historia ve las respuestas, con quién las escribió). Confirmado en el propio código: lo único real que existía sobre una historia ajena era "responder" (0071_message_story_reply.sql), que manda un mensaje de chat NORMAL citándola -- nada equivalente a una respuesta estructurada y privada a una pregunta concreta.
  ...
  - **Verificado real, no simulado**: RLS 340/340. Android `:app:compileDebugKotlin` OK a la primera (`BUILD SUCCESSFUL`), daemon detenido. iOS: pendiente de verificar en CI real tras el push (próxima entrada de esta misma ronda) -- ronda con más reestructuración de Swift que de costumbre, sin verificación local de compilador.
