...
- **Ronda 2026-08-26 (dentro de `/loop`), videollamada de GRUPO real, comparado con WhatsApp/Messenger/Telegram**: las tres dejan llamar a TODO un grupo desde el propio chat de grupo, no solo 1:1. Hueco documentado explícitamente como aplazado en la propia 0079_calls.sql ("llamadas de GRUPO quedan fuera de esta ronda... hueco real futuro, no construido a medias") -- `calls` era estrictamente 1:1 (`chat_id`→`chats`, que a su vez siempre es 1:1, y `callee_id` una sola persona).
  ...
  - **Verificado real, no simulado**: RLS 251/251. Android `:app:compileDebugKotlin` OK (`BUILD SUCCESSFUL`, tras dos fallos reales de compilación propios corregidos en el camino: faltaba `import kotlinx.coroutines.flow.update` para el operador `by`/`.update{}` de `MutableStateFlow`, y `Icons.Filled.Call`/`Videocam` no se resuelven vía un `androidx.compose.material.icons.Icons.Filled.X` completamente cualificado -- hace falta importar la propiedad de extensión en sí), daemon detenido. iOS: pendiente de verificar en CI real tras el push (próxima entrada de esta misma ronda).
