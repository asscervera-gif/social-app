...
- **Ronda 2026-08-26 (dentro de `/loop`), fijar un chat arriba de la lista, comparado con WhatsApp/Telegram/Messenger**: las tres dejan anclar una conversación (1:1 o de grupo) arriba de "Tus chats" manteniendo pulsado. Confirmado en el propio código: `chats` solo tenía `hidden_by_a/b` (0044) y `muted_by_a/b` (0047) -- ninguna columna de "fijado". Extensión directa del mismo patrón "dos columnas booleanas por participante + trigger que impide tocar la del otro" ya usado dos veces en `chats` y una vez más en `group_chat_members` (0064) -- sin RLS nueva en ningún caso.
  ...
  - **Verificado real, no simulado**: RLS 233/233. Android `:app:compileDebugKotlin` OK a la primera (`BUILD SUCCESSFUL`), daemon detenido. **iOS: pendiente de confirmar CI en verde tras este push.**
