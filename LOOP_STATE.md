...
- **Ronda 2026-08-25 (dentro de `/loop`), Reels: primera UI de cliente real sobre el backend de la ronda anterior (0050_reels.sql, tabla+RLS+contadores+avisos ya verificados con 79/79 tests, pero SIN ningún punto de la interfaz que los usara hasta ahora)**. Primer reproductor de vídeo real de toda la app en cualquiera de las dos plataformas -- ni Historias, ni chat multimedia, ni publicaciones normales reproducen vídeo, todo era foto.
  ...
  - **Verificado real, no simulado**: Android `:app:compileDebugKotlin` OK, incluida la resolución real de las dos dependencias nuevas de Media3. iOS: verificación final pendiente del CI real de este push.
  - **Pendiente real explícito**: comentarios de reels (ver el contador real `comment_count`) se quedan sin pantalla para escribir/leer -- alcance deliberado de esta pasada, ya documentado en vez de fingido; "En directo" (streaming en vivo) sigue pendiente de decidir un motor real (LiveKit u otro) antes de escribir código de cliente.
