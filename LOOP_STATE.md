...
- **Ronda 2026-08-26 (dentro de `/loop`), desactivar los comentarios de una publicación/reel, comparado con Instagram/TikTok**: las dos dejan al autor cerrar los comentarios de UNA publicación concreta sin borrarla ni bloquear/silenciar a nadie -- los comentarios que ya existían se quedan tal cual, solo se cierra la puerta a comentarios NUEVOS. Confirmado en el propio código: `grep` de "comments_disabled"/"disable.*comment"/"allow_comments" en todo el repo no encontró nada -- la única forma real de "cerrar" los comentarios era borrar la publicación entera.
  ...
  - **Verificado real, no simulado**: RLS 271/271. Android `:app:compileDebugKotlin` OK a la primera (`BUILD SUCCESSFUL`), daemon detenido. iOS: pendiente de verificar en CI real tras el push (próxima entrada de esta misma ronda).
