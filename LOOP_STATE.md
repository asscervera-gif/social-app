...
- **Ronda 2026-08-27 (dentro de `/loop`), etiqueta de ubicación en una publicación, comparado con Instagram/Facebook/Twitter/Snapchat**: las cuatro dejan añadir un nombre de sitio real a una publicación, mostrado bajo el nombre del autor. Confirmado en el propio código: `posts` no tenía ninguna columna de ubicación -- lo único parecido (`profiles.last_lat/last_lng`, `events.venue_lat/venue_lng`) no está pensado para etiquetar UNA publicación puntual.
  ...
  - **Verificado real, no simulado**: RLS 322/322. Android `:app:compileDebugKotlin` OK a la primera (`BUILD SUCCESSFUL`), daemon detenido. iOS: pendiente de verificar en CI real tras el push (próxima entrada de esta misma ronda).
