...
- **Ronda 2026-08-27 (dentro de `/loop`), "¿Quién puede comentar?", comparado con Twitter/X/TikTok**: Twitter/X deja elegir, al publicar, quién puede responder (Todos / Solo a quienes sigues / Solo a quienes mencionas); TikTok tiene su propia versión (Todos / Amigos / Nadie). `comments_disabled` (0086) ya cubría el extremo "Nadie", pero SOCIAL no tenía ningún término medio real entre "todos pueden comentar" y "nadie puede comentar".
  ...
  - **Verificado real, no simulado**: RLS 332/332. Android `:app:compileDebugKotlin` OK a la primera (`BUILD SUCCESSFUL`), daemon detenido. iOS: pendiente de verificar en CI real tras el push (próxima entrada de esta misma ronda).
